...
# ==========================================
# 2. Нормализация (базовый уровень)
# ==========================================
GROUP_n = "2_normalization"
...
# ==========================================
# 2.1 Нормализация возраста
# Есть 2_1: oo, vpo, spo, pk, doshkolka, naselenie
# Нет 2_1:  obshagi_vpo, obshagi_spo, ped_kadry
# doshkolka и naselenie идут с 2_1 сразу на слой 4 (минуя 3)
# ==========================================
GROUP_n_age = "2_normalization_age"

@asset(group_name=GROUP_n_age)
def n_obuch_doshkolka_age(n_obuch_doshkolka): """age_group → age (int), на слой 4"""
@asset(group_name=GROUP_n_age)
def n_naselenie_age(n_naselenie): """age строка → age (int), на слой 4"""
@asset(group_name=GROUP_n_age)
def n_obuch_oo_age(n_obuch_oo): """row_name → age (int)"""
@asset(group_name=GROUP_n_age)
def n_obuch_vpo_age(n_obuch_vpo): """row_name → age (int)"""
@asset(group_name=GROUP_n_age)
def n_obuch_spo_age(n_obuch_spo): """row_name → age (int)"""
@asset(group_name=GROUP_n_age)
def n_obuch_pk_age(n_obuch_pk): """column_name → age (int)"""

# ==========================================
# 3. Агрегация
# С 2_1: a_obuch_oo/vpo/spo/pk ← от n_*_age
# Без 2_1: a_obshagi_vpo/spo, a_ped_kadry ← от n_* напрямую
# a_doshkolka и a_naselenie ОТСУТСТВУЮТ (идут с 2_1 на слой 4)
# ==========================================
GROUP_agg = "3_aggregation"

@asset(group_name=GROUP_agg)
def a_obuch_oo(n_obuch_oo_age): pass
@asset(group_name=GROUP_agg)
def a_obuch_vpo(n_obuch_vpo_age): pass
@asset(group_name=GROUP_agg)
def a_obuch_spo(n_obuch_spo_age): pass
@asset(group_name=GROUP_agg)
def a_obuch_pk(n_obuch_pk_age): pass
@asset(group_name=GROUP_agg)
def a_obshagi_vpo(n_obshagi_vpo): pass
@asset(group_name=GROUP_agg, tags=UNIMPLEMENTED)
def a_obshagi_spo(n_obshagi_spo): """КРАСНЫЙ"""
@asset(group_name=GROUP_agg)
def a_ped_kadry(n_ped_kadry): pass

# ==========================================
# 4. Валидация / Компоновка
# v_1_1        ← n_obuch_doshkolka_age (2_1, минует 3)
# v_naselenie  ← n_naselenie_age       (2_1, минует 3)
# остальные    ← от слоя 3
# ==========================================
GROUP_L2_STUDENTS = "4_valid_obuch"

@asset(group_name=GROUP_L2_STUDENTS)
def v_naselenie_v_tselom(n_naselenie_age): pass      # 2_1 → 4 напрямую
@asset(group_name=GROUP_L2_STUDENTS)
def v_1_1(n_obuch_doshkolka_age): pass               # 2_1 → 4 напрямую
@asset(group_name=GROUP_L2_STUDENTS)
...
@asset(group_name=GROUP_L2_STUDENTS)
def v_2_5_1(a_obuch_spo): pass
@asset(group_name=GROUP_L2_STUDENTS)
def v_2_5_2(a_obuch_spo): pass
@asset(group_name=GROUP_L2_STUDENTS)
...
defs = Definitions(
    assets=[
        ...
        # 2. Нормализация (базовый)
        n_obuch_doshkolka, n_naselenie, n_obuch_oo, n_obuch_vpo,
        n_obuch_spo, n_obuch_pk, n_obshagi_vpo, n_obshagi_spo, n_ped_kadry,

        # 2.1 Нормализация возраста
        n_obuch_doshkolka_age, n_naselenie_age,
        n_obuch_oo_age, n_obuch_vpo_age, n_obuch_spo_age, n_obuch_pk_age,

        # 3. Агрегация (нет a_doshkolka, a_naselenie)
        a_obuch_oo, a_obuch_vpo, a_obuch_spo, a_obuch_pk,
        a_obshagi_vpo, a_obshagi_spo, a_ped_kadry,
