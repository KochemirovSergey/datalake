...
def _normalize_age(row_name: str) -> str | None:
    ...
    # Паттерн для форматов вида 'в том числе в возрасте (число полных лет на 1 января): 26'
    m = re.search(r':\s*(в возрасте моложе \d+ лет|\d{2}[–-]\d{2}|\d+ и старше|\d+)$', s)
    if m:
        return m.group(1).replace('–', '-')

    # Ищем стандартные паттерны
...
def run() -> int:
    ...
    try:
        ...
        if len(existing) > 0:
            log.info("Удаляем %d старых строк из silver.spo", len(existing))
